...
class ListKategori extends StatefulWidget {
  const ListKategori({super.key,required this.title});
  final String title;

  @override
  State<ListKategori> createState() => _BerandaSiswaState();
}

class _BerandaSiswaState extends State<ListKategori> {
  ...
  void loadGuru() async {
    try {
      final response = await Dio().get('${Network().baseUrl}/users/kategori',data: {'mata_pelajaran':widget.title});
      if (response.statusCode == 200) {
        setState(() {
                ...
                return Padding(
                  ...
                  child: Material(
                    ...
                    child: Padding(
                      ...
                      child: Row(
                        children: [
                          dataGuru[i]['foto_profil'].toString().isNotEmpty
                              ? Image.network(
